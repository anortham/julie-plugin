...
function detectPlatform(plat = os.platform(), arch = os.arch()) {
  if (plat === 'darwin' && arch === 'arm64') {
    return {
      ...
      binaryName: 'julie-adapter',
      daemonBinaryName: 'julie-daemon',
  ...
  if (plat === 'win32' && arch === 'x64') {
    return {
      ...
      binaryName: 'julie-adapter.exe',
      daemonBinaryName: 'julie-daemon.exe',
...
function legacyServerPathFor(binaryPath, plat) {
  const legacyName = plat === 'win32' ? 'julie-server.exe' : 'julie-server';
  return path.join(path.dirname(binaryPath), legacyName);
}
...
function resolveLaunchBinary({
  ...
  if (fsImpl.existsSync(binaryPath)) {
    if (!daemonBinaryPath || fsImpl.existsSync(daemonBinaryPath)) {
      return binaryPath;
    }

    if (fsImpl.existsSync(legacyServerPath)) {
      return legacyServerPath;
    }

    if (required) {
      throw new Error(`missing required Julie binary after extraction: ${daemonBinaryPath}`);
    }
    return null;
  }
...
function maybeStopExistingDaemon({
  ...
  // Prefer julie-daemon for the stop call. Older installations (pre-A1.8)
  // shipped julie-server only, which accepts `stop` as a top-level
  // subcommand; fall back to that binary path if julie-daemon is not yet
  // present after extraction. julie-adapter does NOT accept `stop`, so
  // never invoke `binaryPath` (which is julie-adapter) unless we know it
  // points at a legacy julie-server install.
  const stopBinary = daemonBinaryPath && fsImpl.existsSync(daemonBinaryPath)
    ? daemonBinaryPath
    : binaryPath;

  try {
    execFileSyncImpl(stopBinary, ['stop'], {
      stdio: 'pipe',
      windowsHide: true,
    });
  } catch (error) {
...
function prepareBinaryForLaunch({
  ...
  const legacyServerPath = legacyServerPathFor(binaryPath, plat);
  const daemonPathFor = (launchPath) => launchPath === binaryPath ? daemonBinaryPath : null;
  ...
  if (needsExtract) {
    if (launchPath) {
      maybeStopExistingDaemonImpl({
        ...
        daemonBinaryPath: daemonPathFor(launchPath),
        markerPath,
        markerKey,
        fsImpl,
        stderr,
  ...
  if (needsExtract) {
    ...
    extractBinaryImpl({
      ...
      plat,
      touchPaths: [binaryPath, daemonBinaryPath, legacyServerPath].filter(Boolean),
      stderr,
      fsImpl,
    });
...
function main() {
  ...
  const { target, binaryName, daemonBinaryName, archivePattern } = runtime;
  const binaryPath = path.join(pluginRoot, 'bin', target, binaryName);
  const daemonBinaryPath = daemonBinaryName
    ? path.join(pluginRoot, 'bin', target, daemonBinaryName)
    : null;
