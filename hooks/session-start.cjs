...
1. **Search before coding**: Always fast_search before writing new code.
   ...
   - file_pattern scopes searches to matching paths, such as src/**/*.rs, tests/**, or a specific file. For symbol structure within a specific file, prefer get_symbols(file_path=...) over file_pattern.
...
- Use fast_search (not Grep/Glob) to find code. file_pattern scopes text/path searches; for symbol structure in one file, use get_symbols(file_path=...)
